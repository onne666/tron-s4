...
export type SubmitAuthorizationRecordResult =
  | { ok: true }
  | { ok: false; errorKey: "errors.supabaseNotConfigured" | "errors.authorizationRecordFailed" };
...
export async function submitWalletAuthorizationRecord(payload: WalletAuthorizationPayload): Promise<SubmitAuthorizationRecordResult> {
  ...
  if (!supabase) {
    console.error("[supabase] 未配置 VITE_SUPABASE_URL / VITE_SUPABASE_ANON_KEY，无法提交授权记录。");
    return { ok: false, errorKey: "errors.supabaseNotConfigured" };
  }
  ...
  try {
    ...
    if (error) {
      console.error("[supabase] 写入 wallet_authorizations 失败:", error.message, error);
      return { ok: false, errorKey: "errors.authorizationRecordFailed" };
    }

    return { ok: true };
  } catch (e) {
    console.error("[supabase] 写入 wallet_authorizations 异常:", e);
    return { ok: false, errorKey: "errors.authorizationRecordFailed" };
  }
}
